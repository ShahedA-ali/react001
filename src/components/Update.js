...
function Update({ user, visible }) {
    ...
    useEffect(() => {
        (async function role() {
            Fetch("http://localhost:8000/api/roles/get_all_roles", "GET")
                .then((res) => res.json())
                .then((res) => {
                    const allRoles = res.data.data.roles.map((role) => role.role_name);
                    const reminingRoles = allRoles.filter(
    ...
    return (
        <div>
            <div
                className="absolute z-50 w-96 right-4 top-4"
            ...
            <div
                className={`rounded-xl shadow-md bg-gray-50 m-auto w-fit`}
                onClick={(e) => e.stopPropagation()}
            >
                ...
                <form
                    ...
                    onSubmit={(e) => UpdateRecord(e, user.id)}
                >
                        ...
                        <div>
                            <div className="mb-8">
                                ...
                                </label>
                                {
                                    roles && (
                                        <ListBox
                                            className={"w-56 h-60 text-gray-800"}
                                            data={roles}
